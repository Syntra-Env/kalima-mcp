 ...
 */
export async function getNextVerseInWorkflow(session_id: string): Promise<{
  success: boolean;
  message: string;
  verse?: { surah: number; ayah: number; text: string };
  ...
  try {
    ...
    if (status === 'completed') {
      return {
        ...
        progress: {
          current: totalVerses as number,
          total: totalVerses as number,
          percentage: 100
        },
        completed: true
      };
    ...
    const nextIndex = currentIndex as number;

    if (nextIndex >= verses.length) {
      // Mark session as completed
      db.exec(
        'UPDATE workflow_sessions SET status = ?, current_index = ? WHERE session_id = ?',
        ['completed', nextIndex, session_id]
      );
      saveDatabase(db);
    ...
    const percentage = Math.round((nextIndex / (totalVerses as number)) * 100);
...
export async function submitVerification(options: {
  ...
  message: string;
  evidence_id?: string;
  next_verse?: { surah: number; ayah: number; text: string };
  progress: { current: number; total: number; percentage: number };
  completed: boolean;
}> {
  const db = await getDatabase();
  ...
  try {
    ...
    db.exec(
      'UPDATE workflow_sessions SET current_index = ? WHERE session_id = ?',
      [nextIndex, options.session_id]
    );
    ...
    if (nextIndex >= verses.length) {
      db.exec(
        'UPDATE workflow_sessions SET status = ? WHERE session_id = ?',
        ['completed', options.session_id]
      );
      saveDatabase(db);
 ...
 */
export async function getWorkflowStats(session_id: string): Promise<{
  success: boolean;
  message: string;
  stats?: {
    total_verses: number;
  ...
  try {
    ...
    const verificationCounts: VerificationResult = {
      supports: 0,
      contradicts: 0,
      unclear: 0,
      total_verified: 0
    ...
    if (verificationResult.length > 0) {
      verificationResult[0].values.forEach(row => {
        ...
        if (verification === 'supports') verificationCounts.supports = count as number;
        if (verification === 'contradicts') verificationCounts.contradicts = count as number;
        if (verification === 'unclear') verificationCounts.unclear = count as number;
      });
    }
    ...
    const percentageComplete = Math.round(
      ((currentIndex as number) / (totalVerses as number)) * 100
    );
    ...
    return {
      ...
      stats: {
        total_verses: totalVerses as number,
        verified: currentIndex as number,
        remaining: (totalVerses as number) - (currentIndex as number),
...
export async function listWorkflowSessions(options?: {
  ...
  try {
    ...
    const sessions = result[0].values.map(row => {
      ...
      const percentage = Math.round(((currentIndex as number) / (totalVerses as number)) * 100);

      return {
        session_id: sessionId as string,
        claim_id: claimId as string,
        workflow_type: workflowType as string,
        created_at: createdAt as string,
        progress: {
          current: currentIndex as number,
          total: totalVerses as number,
          percentage
        },
 ...
 */
export async function checkAndTransitionPhase(session_id: string): Promise<{
  success: boolean;
  message: string;
  phase_transition?: { from: string; to: string; reason: string };
}> {
  const db = await getDatabase();
  ...
  try {
    ...
    const sessionResult = db.exec(
      'SELECT claim_id FROM workflow_sessions WHERE session_id = ?',
      [session_id]
    );
    ...
    // Get current phase
    const claimResult = db.exec(
      'SELECT phase FROM claims WHERE id = ?',
      [claimId]
    );
    ...
    const currentPhase = claimResult[0].values[0][0] as string;
    ...
    if (newPhase && newPhase !== currentPhase) {
      db.exec(
        'UPDATE claims SET phase = ? WHERE id = ?',
        [newPhase, claimId]
      );
      saveDatabase(db);
