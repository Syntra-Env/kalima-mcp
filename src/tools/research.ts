...
export async function getClaimEvidence(claim_id: string): Promise<Evidence[]> {
  ...
  return rowsToObjects<Evidence>(result[0].columns, result[0].values);
}

export async function getClaimDependencies(claim_id: string): Promise<{
  claim: Claim | null;
  dependencies: Array<{ claim: Claim; type: string }>;
}> {
  const db = await getDatabase();
  ...
  const claims = rowsToObjects<Claim>(claimResult[0].columns, claimResult[0].values);
  ...
  const rawDeps = rowsToObjects<Claim & { dep_type: string }>(
    depsResult[0].columns,
    depsResult[0].values
  );
...
export async function listPatterns(pattern_type?: string): Promise<Pattern[]> {
  const db = await getDatabase();
  ...
  return rowsToObjects<Pattern>(result[0].columns, result[0].values);
}
...
export async function saveInsight(data: {
  ...
  phase?: string;
  pattern_id?: string;
  evidence_verses?: Array<{ surah: number; ayah: number; notes?: string }>;
  ...
  try {
    ...
    db.run(
      'INSERT INTO claims (id, content, phase, pattern_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
      [claim_id, content, phase, pattern_id || null, now, now]
    );
    ...
    for (const evidence of evidence_verses) {
      ...
      db.run(
        ...
        [evidence_id, claim_id, evidence.surah, evidence.ayah, evidence.notes || null, now]
      );
    }
...
export async function deleteMultipleClaims(claim_ids: string[]): Promise<{ success: boolean; deleted: number; failed: string[]; message: string }> {
  const db = await getDatabase();
  const failed: string[] = [];
  let deleted = 0;
