...
# --- Dihedral Group Generators (Section 3.2) ---
...
def get_incompatibility(x: int, y: int) -> float:
    ...
    Measures the geometric tension between two addresses in the manifold.
    """
    dh = dist_hamming(x, y) / 256.0
    # Normalize dR by max possible distance (MOD/2)
...
def get_dihedral_orbit(x: int) -> dict[str, int]:
    ...
    return {
        ...
        "neg": neg(x),
        "bnot": bnot(x),
        "succ_bnot": neg(bnot(x)) # which is succ(x)
    }

def decompose_fibers(x: int) -> list[int]:
    \"\"\"Z/2Z fibration of ring elements into 256 binary fibers (P3.3).
    
    Returns a list of 256 bits, where fibers[k] is the k-th bit of x.
    \"\"\"
...
def get_stratum(x: int) -> int:
    ...
    The stratum is the index of the first non-zero bit.
    \"\"\"
    if x == 0: return 256
