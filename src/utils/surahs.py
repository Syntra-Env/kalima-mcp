"""Surah names constant — replaces the surahs database table."""
...
def get_surah_name(number: int) -> Optional[str]:
    """Get surah name by number. Returns None if invalid."""
    return SURAH_NAMES.get(number)


def list_all_surahs() -> list[dict]:
    """List all 114 surahs with number and name."""
    return [{"number": n, "name": name} for n, name in sorted(SURAH_NAMES.items())]
