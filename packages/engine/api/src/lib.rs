...
pub async fn start_server_with_config(config: ServerConfig) {
    ...
    let app = Router::new()
        ...
        // Claims endpoints (QuranResearch integration)
        .route("/api/claims", get(handlers::research::list_claims))
        .route("/api/claims/:claim_id/evidence", get(handlers::research::get_claim_evidence))
        .route("/api/claims/:claim_id/dependencies", get(handlers::research::get_claim_dependencies))
        .route("/api/research/patterns", get(handlers::research::list_patterns))
