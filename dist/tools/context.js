 ...
 */
export async function getVerseWithContext(surah, ayah, options = {}) {
    const db = await getDatabase();
    const { include_root_claims = true, include_form_claims = true, include_pos_claims = true } = options;
    // Get the verse text
    const verseResult = db.exec('SELECT surah_number, ayah_number, text FROM verse_texts WHERE surah_number = ? AND ayah_number = ?', [surah, ayah]);
    if (!verseResult.length || !verseResult[0].values.length) {
        throw new Error(`Verse ${surah}:${ayah} not found`);
    }
    const verse = rowsToObjects(verseResult[0].columns, verseResult[0].values)[0];
    ...
    ORDER BY t.token_index ASC`, [surah, ayah]);
    ...
    for (const token of tokenData) {
        ...
        for (const feature of featuresToCheck) {
            if (feature.value) {
                const claimsResult = db.exec(`SELECT DISTINCT
            c.id as claim_id,
          ...
          WHERE plf.feature_type = ? AND plf.feature_value = ?`, [feature.type, feature.value]);
                if (claimsResult.length && claimsResult[0].values.length) {
                    const claims = rowsToObjects(claimsResult[0].columns, claimsResult[0].values);
    ...
    const verseClaimsResult = db.exec(`SELECT
      vc.claim_id,
      c.content as claim_content,
      c.phase as claim_phase,
      vc.evidence_type
    ...
    const directVerseClaims = verseClaimsResult.length && verseClaimsResult[0].values.length
        ? rowsToObjects(verseClaimsResult[0].columns, verseClaimsResult[0].values)
        : [];
    return {
        verse,
